...
class _AvailableDriversScreenState extends State<AvailableDriversScreen>
  ...
  void initState() {
    ...
    // Start listening to drivers immediately — don't wait for pickup geocode
    _startDriverStream();
    _resolvePickup();
  }
  ...
  /// Stream all drivers right away, sort by distance once pickup is known
  void _startDriverStream() {
    _driverSub = db.collection('drivers')
        .where('isOnline', isEqualTo: true)
        .snapshots().listen((snap) {
      _rebuildDriverList(snap.docs);
    });
  }

  void _rebuildDriverList(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
    ...
    for (final doc in docs) {
      ...
      final distKm = (hasLocation && _pickupLatLng != null)
          ? MapService.distanceKm(
              _pickupLatLng!.latitude, _pickupLatLng!.longitude, lat!, lng!)
          : double.maxFinite;
  ...
  Future<void> _resolvePickup() async {
    ...
    if (widget.pickupLat != null && widget.pickupLng != null) {
      ...
      final snap = await db.collection('drivers').where('isOnline', isEqualTo: true).get();
      _rebuildDriverList(snap.docs);
      _fitMapBounds();
      return;
    }
    ...
    try {
      ...
      if (loc != null) {
        latlng = LatLng((loc['lat'] as num).toDouble(),
    ...
    if (latlng == null) {
      ...
      if (pos != null) latlng = LatLng(pos.latitude, pos.longitude);
    }
    ...
    final snap2 = await db.collection('drivers').where('isOnline', isEqualTo: true).get();
    _rebuildDriverList(snap2.docs);
    _fitMapBounds();
  }
  ...
  Future<void> _bookDriver(
      _DriverInfo driver, StateSetter setSt) async {
    setSt(() => _booking = true);
    if (mounted) setState(() => _booking = true);
    try {
      final driverDoc =
          await db.collection('drivers').doc(driver.id).get();
                                ...
                                (i) => Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    color: _navy.withOpacity(
                                        0.2 + 0.6 * (((_pulseCtrl.value + i * 0.33) % 1.0))),
                                  ),
                                ),
                              ),
                            ),
                    ...
                    SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        padding: EdgeInsets.only(
                            left: 16, right: 16, bottom: bottomPad),
      ...
      child: AnimatedContainer(
        ...
        children: [
          ...
          Text(
            driver.name,
            style: const TextStyle(
                color: _navy, fontWeight: FontWeight.bold, fontSize: 12),
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Text(sublabel,
                style: TextStyle(
                    color: _navy.withOpacity(0.45), fontSize: 10)),
          ],
        ),
      ),
    );
