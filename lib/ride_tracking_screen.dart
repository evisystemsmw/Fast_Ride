...
class _RideTrackingScreenState extends State<RideTrackingScreen> {
  ...
  void _listenRide() {
    _rideSub = db.collection('rides').doc(widget.rideId).snapshots().listen(
      (snap) {
        ...
        // when trip starts, redraw route to destination
  ...
  void _listenAssignedDriver(String driverId) {
    _driversSub?.cancel();
    _driversSub = db.collection('drivers').doc(driverId).snapshots().listen((snap) {
      if (!snap.exists || !mounted) return;
  ...
  Future<void> _updateLiveRoute(LatLng driverPos) async {
    // throttle: only re-draw if driver moved > 30 m
    if (_lastDriverPos != null) {
      ...
      if (distSq < 0.000000081) return; // ~30 m threshold
    }
  ...
  Future<void> _calculateRoute() async {
    // Geocode pickup and destination to get coordinates + draw route + estimate fare
    try {
      final pickupRes = await http.get(Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json'
        '?address=${Uri.encodeComponent(widget.pickup)}'
        '&key=$geocodingApiKey',
      ));
      final destRes = await http.get(Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json'
        '?address=${Uri.encodeComponent(widget.destination)}'
        '&key=$geocodingApiKey',
      ));

      final pickupLoc = jsonDecode(pickupRes.body)['results']?[0]
          ?['geometry']?['location'];
      final destLoc = jsonDecode(destRes.body)['results']?[0]
          ?['geometry']?['location'];

      if (pickupLoc == null || destLoc == null) return;

      final origin = LatLng(pickupLoc['lat'], pickupLoc['lng']);
      final dest = LatLng(destLoc['lat'], destLoc['lng']);
      ...
      setState(() {
        _markers = {
          ...
          Marker(
            ...
            infoWindow:
                InfoWindow(title: 'Destination: ${widget.destination}'),
          ),
        };
      });

      // Draw route
      final dirRes = await http.get(Uri.parse(
        'https://maps.googleapis.com/maps/api/directions/json'
      ...
      if ((dirData['routes'] as List).isEmpty) return;
  ...
  Future<void> _cancelRide() async {
    ...
    // listener will detect 'cancelled' status and pop — no double-pop here
  }
  ...
  String get _statusLabel {
    ...
    switch (_rideStatus) {
      ...
      case 'in_trip':   return 'Trip in progress';
  ...
  Color get _statusColor {
    if (_rideStatus == 'accepted' && _tripPhase == 'arrived') return Colors.orange;
    switch (_rideStatus) {
      case 'requested': return Colors.orange;
      case 'accepted':  return Colors.blue;
      case 'in_trip':   return Colors.green;
  ...
  IconData get _statusIcon {
    if (_rideStatus == 'accepted' && _tripPhase == 'arrived') return Icons.place_rounded;
    switch (_rideStatus) {
      case 'accepted':  return Icons.directions_car_rounded;
      case 'in_trip':   return Icons.navigation_rounded;
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Container(
                          width: 1, height: 40, color: _navy.withOpacity(0.1)),
                      Expanded(
                        child: _InfoTile(
  ...
  void _showCompletedSheet() {
    final fare = _rideData?['finalFare'] as num?;
    showModalBottomSheet(
      ...
      builder: (_) => _RatingSheet(
        fare: fare?.toDouble() ?? _estimatedFare,
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Text(label,
            style:
                TextStyle(color: _navy.withOpacity(0.45), fontSize: 11)),
      ],
    );
  }
}
...
class _RatingSheetState extends State<_RatingSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final driverId = (await db.collection('rides').doc(widget.rideId).get())
      ...
      if (driverId != null) {
        ...
        double sum = 0;
        int count = 0;
        for (final r in ridesSnap.docs) {
          final ratingVal = (r.data()['driverRating'] as num?)?.toDouble() ?? 0;
          if (ratingVal > 0) { sum += ratingVal; count++; }
        }
        if (count > 0) {
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text('MWK ${widget.fare.toStringAsFixed(0)}',
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: TextStyle(color: _navy.withOpacity(0.35), fontSize: 13),
                filled: true,
            ...
            TextButton(
              ...
              child: Text('Skip', style: TextStyle(color: _navy.withOpacity(0.4), fontSize: 13)),
            ),
          ],
        ),
        ),
