...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
  ...
  Future<void> _drawRoute(LatLng origin, LatLng dest) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (routes != null && routes.isNotEmpty) {
          final distanceM = (routes[0]['legs'][0]['distance']['value'] as num).toDouble();
    ...
    // Fallback: straight-line distance so fare is never stuck at "Calculating..."
    if (!mounted) return;
    setState(() {
      _distanceKm = MapService.distanceKm(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if ((widget.ride['passengerName'] ?? '').toString().isNotEmpty)
                    Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          if ((widget.ride['passengerPhone'] ?? '').toString().isNotEmpty)
                            GestureDetector(
                              onTap: () => launchUrl(Uri.parse('tel:${widget.ride['passengerPhone']}')),
                              child: Container(
                        ...
                        child: ElevatedButton.icon(
                          onPressed: () async {
                            ...
                            await db.collection('rides').doc(widget.rideId).update({
                              ...
                              'driverCar': '${dd['vehicleMake'] ?? ''} ${dd['numberPlate'] ?? ''}'.trim(),
                              'status': 'accepted',
                              if (_distanceKm > 0) 'estimatedDistanceKm': _distanceKm,
                              if (_distanceKm > 0) 'estimatedFare': _estimatedFare,
                            });
                            if (context.mounted) Navigator.pop(context);
                          },
